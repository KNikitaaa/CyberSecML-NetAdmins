test_that("run_pipeline_from can normalize from supplied raw inputs and persist stage status", {
  ...
  expect_true(any(result$pipeline_status$stage == "normalize"))
  expect_equal(result$pipeline_status$status[result$pipeline_status$stage == "normalize"][[1]], "success")
  expect_true(any(result$pipeline_status$stage == "sanity_checks"))
})
